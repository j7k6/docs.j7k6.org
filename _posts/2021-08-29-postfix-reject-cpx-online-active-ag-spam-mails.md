...
*cpx online active AG* is a notorious spamming company from Switzerland. The unsubscribe option they offer in their mails is a fraud, spam mails keep coming just from different senders. But there is one thing they all have in common: the mail footer always contains the company's name.
To reject all mails containing the string `cpx online active AG`, the Postfix *body_checks* option can be used.

1. Install the Postfix PCRE modle:
   ```bash
